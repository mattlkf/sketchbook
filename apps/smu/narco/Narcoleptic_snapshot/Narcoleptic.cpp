...
// Matt
uint8_t narcoleptic_using_interrupts = 0; // flag to see if interrupts have been enabled
volatile uint8_t narcoleptic_interrupted; // counter to track how many times interrupt fired

void isr_0(void){
  ++narcoleptic_interrupted;
  detachInterrupt(0);
}
...
void NarcolepticClass::useInterrupt(uint8_t num, uint8_t mode){
  narcoleptic_using_interrupts = 1;
  isr_num = num;
  isr_mode = mode;

  if(num == 0) pinMode(2, INPUT);
  else if(num == 1) pinMode(3, INPUT);
}
void NarcolepticClass::setInterrupt(){
  if(isr_num == 0){
    attachInterrupt(0, isr_0, isr_mode);
  }
  else if(isr_num == 1){
    attachInterrupt(1, isr_1, isr_mode);
  }
}

uint8_t NarcolepticClass::wasInterrupted(){
  return narcoleptic_interrupted;
}
...
void NarcolepticClass::sleep(uint8_t wdt_period,uint8_t sleep_mode) {
  ...
  //Matt
  if(narcoleptic_using_interrupts){
    setInterrupt();
  }
...
void NarcolepticClass::delay(int milliseconds) {
  ...
  if (microseconds > 0) {
    ...
    narcoleptic_interrupted = 0; // Matt
    while (sleep_periods >= 512 && !narcoleptic_interrupted) {
      sleep(WDTO_8S,SLEEP_MODE_PWR_DOWN);
      sleep_periods -= 512;
    }
    if (!narcoleptic_interrupted && sleep_periods & 256) sleep(WDTO_4S,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 128) sleep(WDTO_2S,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 64) sleep(WDTO_1S,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 32) sleep(WDTO_500MS,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 16) sleep(WDTO_250MS,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 8) sleep(WDTO_120MS,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 4) sleep(WDTO_60MS,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 2) sleep(WDTO_30MS,SLEEP_MODE_PWR_DOWN);
    if (!narcoleptic_interrupted && sleep_periods & 1) sleep(WDTO_15MS,SLEEP_MODE_PWR_DOWN);
  }
}
