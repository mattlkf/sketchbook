...
#define OLED_PIN 9 // this should never be powered down
...
int main(){
    ...
    while(1){
        ...
        // RF24 stuff
            ...
            hum_packet p;
            p.pkt_type = 1; // humidity packet type
            p.t = t;
            p.h = h;
            p.pkt_num = pkt_num++;
            p.src_id = node_id;
...
// Display data
    ...
    // Take a set of readings 
        ...
        // Write data in packet format
        hum_packet p;
        p.pkt_type = 2; // humidity packet type - but for when it's been interrupted
        p.t = t;
        p.h = h;
        p.pkt_num = pkt_num++;
        p.src_id = node_id;
